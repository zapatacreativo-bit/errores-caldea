                            ...
                            >
                                <div className="flex flex-col md:flex-row items-start justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button
                                            type="button"
                                            onClick={() => toggleFix(item.id, item.status)}
                                            disabled={updating === item.id || item.status === 'ignored'}
                                            aria-pressed={item.status === 'fixed'}
                                            style={{ WebkitTapHighlightColor: 'transparent', touchAction: 'manipulation', cursor: 'pointer' }}
                                            className={`inline-flex items-center select-none px-4 py-2 rounded-lg border transition-all active:scale-95 ${item.status === 'fixed'
                                                ? 'bg-green-500/20 border-green-500/30 text-green-300 shadow-[0_0_10px_rgba(34,197,94,0.2)]'
                                                : 'bg-white/5 border-white/10 text-gray-400 hover:bg-white/10 hover:border-white/20'
                                                } disabled:opacity-40 disabled:cursor-not-allowed`}
                                        >
                                            {item.status === 'fixed' ? <Check className="w-4 h-4 mr-2" /> : <div className="w-4 h-4 mr-2 border-2 border-current rounded-sm flex-shrink-0" />}
